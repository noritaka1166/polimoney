...
## このマニュアルについて
このマニュアルでは、政治家の収支報告書を表示する新しいデモページを作成する方法を説明します。既存の藤崎剛暉氏のページ（[`frontend/data/demo-kokifujisaki.ts`](../data/demo-kokifujisaki.ts)）を参考として、同じような構造のページを作る手順を詳しく解説します。
...
### 1. データファイル（.tsファイル）
**場所**: `frontend/data/demo-{政治家名}.ts`
**役割**: 政治家の情報と収支データをすべて格納するファイル

### 2. ページファイル（page.tsxファイル）
**場所**: `frontend/app/demo-{政治家名}-{年度}/page.tsx`
**役割**: 実際にブラウザに表示されるページの構造を定義するファイル

### 3. 画像ファイル（.jpgファイル）
**場所**: `frontend/public/demo-{政治家名}.jpg`
**役割**: 政治家の顔写真
...
#### ファイル名の決定
`frontend/data/demo-{政治家名}.ts`という名前でファイルを作成します。
例：田中太郎という政治家の場合 → `frontend/data/demo-tanakatarou.ts`
...
#### ディレクトリの作成
`frontend/app/demo-{政治家名}-{年度}/`というフォルダを作成します。
例：田中太郎の2024年データの場合 → `frontend/app/demo-tanakatarou-2024/`
...
- **配置場所**: `frontend/public/`フォルダ直下

#### 画像の注意点
- 政治家の顔がはっきりと識別できる写真を使用
- 著作権に問題がない画像を使用
- 画像が無い場合は `frontend/public/demo-example.png` で代替を推奨
...
- 画像ファイルが`frontend/public/`フォルダに正しく配置されているか確認
...
### 参考ファイル
- **データ構造の参考**: [`frontend/data/demo-kokifujisaki.ts`](../data/demo-kokifujisaki.ts)
- **ページ実装の参考**: [`frontend/app/demo-koki-fujisaki-2024/page.tsx`](../app/demo-koki-fujisaki-2024/page.tsx)
- **型定義の確認**: [`frontend/models/type.d.ts`](../models/type.d.ts)
