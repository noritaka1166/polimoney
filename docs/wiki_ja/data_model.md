...
データモデルは`frontend/models/type.d.ts`ファイルで定義されており、以下の主要なデータ型が含まれています：
...
外部データソースからのデータは、アプリケーションで使用される形式に変換する必要があります。この変換プロセスは`frontend/data/converter.ts`で定義されています。
