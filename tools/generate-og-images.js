...
async function captureGraph(slug) {
  ...
  try {
    ...
    // グラフ要素を待機
    await page
      .waitForSelector('.sankey-chart', { timeout: 10000 })
      .catch(() =>
        console.log('Sankey chart not found, trying alternative selector...'),
      );

    // グラフ要素を取得
    const graphElement =
      (await page.$('.sankey-chart')) ||
      (await page.$('svg')) ||
...
async function main() {
  // 出力ディレクトリを作成
  const outputDir = path.join(__dirname, '../frontend/public/ogp');
